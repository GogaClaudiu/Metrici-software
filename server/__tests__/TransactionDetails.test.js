...
describe('TransactionDetails', () => {
  ...
  describe('getTransactionsBetweenDates', () => {
    test('should fetch transactions between two dates', async () => {
      ...
      const resultPromise = transactionDetails.getTransactionsBetweenDates(
        startDate,
        endDate
      );
      ...
      expect(Database.query).toHaveBeenCalledWith(
        expect.stringMatching(
          /\s*SELECT \*\s*FROM Transactions WHERE\s*transactionDate BETWEEN \? AND \?;\s*/
        ),
        [startDate, endDate]
      );
    });
    test('should fetch transactions greater than a given date', async () => {
      ...
      const resultPromise =
        transactionDetails.getTransactionsGreaterThanDate(fromDate);
    ...
    test('should fetch transactions lower than a given date', async () => {
      ...
      const resultPromise =
        transactionDetails.getTransactionsLowerThanDate(toDate);

      await expect(resultPromise).resolves.toEqual(mockedRows);

      expect(Database.query).toHaveBeenCalledWith(expect.any(String), [toDate]);
    });
    test('should fetch transactions within a given amount range', async () => {
      ...
      const resultPromise = transactionDetails.getTransactionsByAmountRange(
        minAmount,
        maxAmount
      );
    ...
    test('should fetch transactions lower than a given amount', async () => {
      ...
      const resultPromise =
        transactionDetails.getTransactionsLowerThanAmount(amount);

      await expect(resultPromise).resolves.toEqual(mockedRows);

      expect(Database.query).toHaveBeenCalledWith(expect.any(String), [amount]);
    });
    test('should fetch transactions greater than a given amount', async () => {
      ...
      const resultPromise =
        transactionDetails.getTransactionsGreaterThanAmount(amount);
